def tests_products(first_product, second_product, third_product, fourth_product):
    ...
    assert first_product.get_products == 180000.0
    assert first_product.quantity == 5

    assert second_product.name == "Iphone 15"
    assert second_product.description == "512GB, Gray space"
    assert second_product.get_products == 210000.0
    assert second_product.quantity == 8

    assert third_product.name == "55 QLED 4K"
    assert third_product.description == "Фоновая подсветка"
    assert third_product.get_products == 123000.0
    ...
    assert fourth_product.get_products == 31000.0
